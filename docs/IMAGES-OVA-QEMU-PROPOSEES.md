...
Pour un TP pédagogique **stable, propre et reproductible** :

- 🛡️ **pfSense** : ISO officiel pfSense CE, puis export OVA après configuration.
...
| 🖥️ VM | ✅ Image conseillée | 📦 Format |
|---|---|---|
| 🛡️ **pfSense** | pfSense CE officiel via Netgate Installer | ISO / [OVA](https://mega.nz/file/f15BVSjI#eO5j5LTHVih6ZJYeDwh_Ljq4gOmcVg42G-viHiTPHk4) |
...
- ❌ Images OVA trouvées sur des sites non officiels, **sauf celles proposées ici par votre formateur**.
...
- 🔐 Vérifier l'origine de l'image avant import.
- 🧾 Documenter la version utilisée : OS, date, source, format.
- 📸 Créer un snapshot propre avant toute modification.
- 🌐 Vérifier la configuration réseau : WAN, LAN, IP fixe, passerelle et DNS.
- 🧪 Tester l'accès console et SSH avant de lancer les TPs.

## 🧩 Formats utiles

| 📦 Format | 🧠 Usage recommandé |
|---|---|
| `.iso` | Installation propre depuis zéro |
| `.ova` | Import rapide dans VMware / VirtualBox |
| `.qcow2` | Usage QEMU / KVM / libvirt |
| `.ovf` / `.vmx` | Import ou compatibilité VMware |
